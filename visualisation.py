...
st.set_page_config(
    page_title="Business Data Visualization Dashboard",
    page_icon="📊",
    layout="wide",
...
if uploaded_file is not None:
    ...
    # Show balloons when file is uploaded
    st.sidebar.balloons()
    ...
    if "Amount" in df.columns:
        ...
        with col1:
            # Interactive elements for category and chart type
            category = st.selectbox(
                "📌 Select Category for Visualization",
                ["Owner", "Customer Name", "Territory Name", "Source", "Industry", "Product"],
                key="category"
            ...
            if category in df.columns:
                ...
                if chart_type == "Histogram":
                    ...
                    fig.update_traces(
                        ...
                        hovertemplate=f"{category}: %{{label}}<br>Amount: %{{value}}"  
                    )
                    fig.update_layout(
                        showlegend=True, 
                        height=600,  
...
# Add more interactivity in the main area
